            ...
            for method in "${method_list[@]}"; do
                for num_words in "${num_words_list[@]}"; do
                    ...
                    if [[ "$method" == "c3d" || "$method" == "cnn_lstm" || "$method" == "mhi_fusion" || "$method" == "mhi_attention" ]]; then
                        python3 main.py --method "$method" --num_words "$num_words" --seed "$seed" --epochs "$epochs" --batch_size "$batch_size" --no_pretrained
                    elif [[ "$method" == "c3d_pretrained" || "$method" == "cnn_lstm_pretrained" ]]; then
                        python3 main.py --method "$method" --num_words "$num_words" --seed "$seed" --epochs "$epochs" --batch_size "$batch_size"
                    else
                        python3 main.py --method "$method" --num_words "$num_words" --seed "$seed"
                    fi
